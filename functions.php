...
if ( ! function_exists( 'Skingym_setup' ) ) {

	function Skingym_setup() {
		...
		// Custom Image Sizes
		add_image_size( 'Skingym_fullscreen', 1860, 9999 );
		...
		// Add nav menu
		register_nav_menu( 'primary-menu', __( 'Primary Menu', 'SkinGym' ) );
		register_nav_menu( 'footer-menu', __( 'Footer Menu', 'SkinGym-Footer' ) );
		register_nav_menu( 'social-menu', __( 'Social Menu', 'SkinGym-Social' ) );
		...
		load_theme_textdomain( 'SkinGym', get_template_directory() . '/languages' );
		
		
		wp_enqueue_style( 'Skingym-style', get_template_directory_uri() . '/style.css', $dependencies );

	}
	add_action( 'after_setup_theme', 'Skingym_setup' );
...
if(isset($_POST['action']) && $_POST['action'] == 'contact_send') {
	    ...
	    $subject = 'Skin-Gym Website - '.$subject;
	    $body = "Name: $name \n\nEmail: $email \n\nPhone: $phone \n\nMessage: $message";
